...
def main():
    ...
    for i in range(len(instructions)):
        ...
        '''
            This moves all the crates from the source to destination in one swoop
            Opposed to one at a time as in Part 1
        '''
        n = len(stacks[source - 1])
        stacks[destination - 1] += stacks[source - 1][n - num_elements:]
        stacks[source - 1] = stacks[source - 1][:n - num_elements]
